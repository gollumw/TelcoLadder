...
| 2026-09-06 | — (DRA specialist; initials not recorded) | not asked | not asked | not asked | — | **A feature request, not a defect report.** Working on a DRA, where the volume is heavy Diameter request/answer traffic across many peers, they want a GUI that correlates **all** sessions at once — "like the call flow". Verified against the code the same day: the engine already draws exactly that shape (`telcoladder analyze` on the DRA fixture renders one ladder per flow with MME, DRA, I-CSCF, S-CSCF, HSS, PCEF and PCRF as lanes), but **the browser ladder is scoped to one subscriber** (`loadCallFlow(supi)`), and flows with no subscriber return `identity: null` from `/flows`, so the browser has no handle for them — CER/CEA and DWR peer maintenance, which is most of what a DRA person looks at, **cannot be opened in the GUI at all**. Not yet built; the open question is whether they mean "let the ladder open any flow" or "a peer-level view that aggregates first and drills second", which are materially different pieces of work at DRA volumes. |
