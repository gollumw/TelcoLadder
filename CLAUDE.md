...
| a segment knows its own messages (`Procedure.members`) and who opened it — a NAS request only a UE can send in the opener makes it `radio` even when the core relays it, "Deregistration request (UE terminated)" makes it `core`, a Detach request follows the sender of the row carrying it, otherwise the opener's sender decides (UE/gNB/eNB → `radio`, other role → `core`, no role → null); selecting a segment shows its members, never its frame range | `procedures._initiator_side`, `callflow._render` (`procedure`) | a UE's PDU session establishment filed under core because its uplink NAS is ciphered and the first readable row is AMF→SMF; a selected registration shows another PDU session or Diameter from the same seconds |
| a radio connection runs from an InitialUEMessage the RAN sent to the first release completion after it (NGAP `UEContextReleaseResponse`, S1AP `UEContextReleaseComplete`), or to just before the next InitialUEMessage marked `released: false`; messages between two connections belong to none | `connections.radio_connections`, `callflow._render` (`connection`) | the Paging that triggers the next connection drawn inside the previous one; an InitialUEMessage the core sent opens a connection |
| a behavior record re-encodes a procedure and adds no wire inference; slowness is judged only in the browser against the user's thresholds — the engine says which threshold and which number (`kpi`), and a failed registration has none; the chain before a failure is message order within the segment and carries no identities; a PathSwitchRequest opens a handover, never a connection | `behavior.py`, `web/src/lib/kpiThresholds.ts` | two slowness verdicts drift apart; a timer-driven registration failure reported as a slow registration; a path switch cuts one connection in two |
