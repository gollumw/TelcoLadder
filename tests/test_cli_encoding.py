...
`telcolens check` 印 `✓`，`analyze` 的摘要印 `⚠`。這兩個字元在 cp950
...
接走時才會炸 —— 也就是 README 教的 `telcolens analyze x.pcap > flow.mmd`。
...
def _run(args: list[str], codepage: str) -> subprocess.CompletedProcess[bytes]:
    return subprocess.run(
        [sys.executable, "-m", "telcolens", *args],
...
def test_check_survives_a_non_utf8_console(codepage):
    """`telcolens check` 的 `✓` 不得因為 console 編碼而讓整個指令當掉。
