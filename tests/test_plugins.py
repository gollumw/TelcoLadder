...
import telcoladder.adapters as adapters_mod
...
def install_plugin(tmp_path, monkeypatch) -> Iterator:
    ...
    def install(
        ...
        *,
        ...
        lines = ["[telcoladder.adapters]", f"faketel = {adapter_target or module}", ""]
        if cause_table:
            lines += ["[telcoladder.cause_tables]",
...
def test_plugin_adapter_lands_in_its_declared_position(install_plugin):
    ...
    assert names == ["ngap", "s1ap", "faketel", "gtpv2", "sbi", "nas-5gs", "nas-eps", "sip", "megaco", "diameter", "sgsap", "enum", "pfcp", "gtp"]


def test_builtins_still_work_when_metadata_cannot_be_enumerated(monkeypatch):
    """列不出套件清單時，內建協定必須照常運作 —— 只警告，不罷工。

    這是「兩種失敗」的另一半（見 telcoladder/plugins.py）：某個外掛壞掉是
    使用者修得掉的問題，該擋在他面前；而 metadata 損壞跟他手上的擷取檔
    無關，沒有任何外掛的 TelcoLadder 仍是完整的 5GC 分析工具。
    ...
    monkeypatch.setattr("telcoladder.plugins.entry_points", boom)
    _clear_caches()
    try:
        ...
        assert names == ["ngap", "s1ap", "gtpv2", "sbi", "nas-5gs", "nas-eps", "sip", "megaco", "diameter", "sgsap", "enum", "pfcp", "gtp"]
...
def test_plugin_dissectors_reach_the_environment_check(install_plugin):
    """外掛宣告的 dissector 要進到 `telcoladder check` 的必要清單。
