...
`-T fields` unusable (see the `telcoladder/extract.py` module docstring) — keep this
...
It is a **second oracle**, independent of both tshark and TelcoLadder: a test can
assert that the AMF logged cause 21 and that TelcoLadder reports the same thing.
tshark and TelcoLadder share a dissector; the AMF does not.

Three-way agreement at the time of capture:

| Source | Says |
|---|---|
| TelcoLadder | `Synch failure (#21) — 3GPP TS 24.501 §9.11.3.2` |
