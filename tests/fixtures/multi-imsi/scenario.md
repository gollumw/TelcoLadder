...
`logs/` is a second oracle, independent of both tshark and TelcoLadder. The AMF log
names all five IMSIs and records five `Registration complete` events — so a test
can assert TelcoLadder found the same five subscribers the core network thinks it
