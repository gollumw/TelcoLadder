...
這一組守的是**兩條強度不同的保證**（見 `telcoladder/prefilter.py` 開頭）：
...
from telcoladder.adapters import display_filter as claimed_filter
from telcoladder.extract import read_frames
from telcoladder.pipeline import Prefilter, analyse
from telcoladder.prefilter import (
    PrefilterError,
    TimeWindow,
    combine,
    narrow_to_identity,
...
def test_the_slice_does_not_survive(e2e_pcap: Path):
    ...
    before = set(Path(tempfile.gettempdir()).glob("telcoladder-slice-*"))
    analyse(
        e2e_pcap, prefilter=Prefilter(window=TimeWindow(None, 1.0)), with_coverage=False
    )
    assert not (set(Path(tempfile.gettempdir()).glob("telcoladder-slice-*")) - before)
