...
**這是預設視圖**（2026-08-17 起）：一格封包一列，載體與載荷堆在同一列
（「DownlinkNASTransport ▸ Authentication request」），整份擷取的節奏一眼掃完。
天天看包的人要的是這個密度。

另一種是流程視圖（`wire=False`）：NGAP 載體與內嵌的 NAS 各畫一支箭，且 NAS
依協定語意畫在 UE↔AMF。它對「想看懂這段程序在做什麼」的人比較好讀，也是本
工具相對於單純封包檢視器的差異化 —— 所以它沒有消失，只是不再是預設。
