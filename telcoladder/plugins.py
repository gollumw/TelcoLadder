...
1. **adapter** —— 怎麼把封包變成 `Message`（`telcoladder.adapters` group）
2. **cause 表** —— 那個協定的 cause code 怎麼查（`telcoladder.cause_tables` group）
...
奇怪的打包方式），跟使用者手上的擷取檔無關。沒有任何外掛的 TelcoLadder
...
ADAPTER_GROUP = "telcoladder.adapters"
CAUSE_TABLE_GROUP = "telcoladder.cause_tables"
