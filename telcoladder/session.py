...
#: 改了之後舊版留下的殘檔就再也不會被回報。
SESSION_PREFIX = "telcoladder-session-"
...
class Session:
    ...
    **與 `filter_frames` 分開存，是因為它們是兩件事。** 一個是「找封包」
    ...
    """使用者自訂的 decode-as 規則（`telcoladder/decodeas.py`）。
        ...
        **不能只在請求進來時順便掃。** 使用者關掉分頁走人之後就不會再有請求，
        ...
        with self._lock:
            ...
            self._reaper = threading.Thread(
                target=self._reap_loop, name="telcoladder-session-reaper", daemon=True
            )
            self._reaper.start()
...
def _delete_if_ours(session: Session) -> None:
    ...
    # `my-telcoladder-session-notes.pcap`（例如從殘檔訊息複製再改名），
