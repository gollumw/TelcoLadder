...
| `DISSECTORS` | `telcoshark check` 要驗證存在的 dissector 名稱 |
...
在這之前 `nas5gs.py` 是直接 `from telcoshark.adapters.ngap import identity_keys` ——
...
from telcoshark.model import IdKey, Message
from telcoshark.plugins import ADAPTER_GROUP, PluginError, load_group
...
from telcoshark.adapters import gtp, nas5gs, ngap, pfcp, sbi  # noqa: E402
...
def _validate(name: str, obj: object) -> Adapter:
    ...
    if missing:
        raise PluginError(
            ...
            f"契約見 telcoshark/adapters/__init__.py。"
        )
...
def required_dissectors() -> tuple[str, ...]:
    """`telcoshark check` 要驗證的 dissector，去重後保持穩定順序。"""
    seen: dict[str, None] = {}
    for adapter in adapters():
