...
const zh_TW: Record<string, string> = {
  ...
  "{n} incomplete": "{n} 通未完成",
  "One row per INVITE dialog (Call-ID, RFC 3261 §8.1.1.4). A number is shown only when the address says it is one - an IMSI-derived IMPU has digits but is not a dialable number, so it stays blank rather than being guessed.": "一個 INVITE dialog 一列（Call-ID，RFC 3261 §8.1.1.4）。**只有位址自己宣告是電話號碼時才顯示號碼** —— IMSI 推導的 IMPU 帶著一串數字卻不是撥得通的號碼，所以留白而不是猜一個。",
