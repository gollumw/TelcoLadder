...
| `sgsap` | ✅ SGsAP on SGs, MME ↔ MSC/VLR (2026-09-12): combined-attach location updates, CS fallback paging, SMS over SGs, IMSI detach. Every message carries the IMSI, so it joins the subscriber's flow; roles come from the message type (Reset and Status, which either side may send, get none). Its own *SGs (CSFB/SMS)* tab, and the *Access & Mobility (S1-MME)* tab shows it too. No cause table yet |
...
| `sip` / `diameter` | ✅ SIP (Gm; keyed on `From`, never `To`) and Diameter (S6a/S6d, Cx/Dx, Gx, Rx, Sh, S6b, SWx with roles and causes; other applications decode with their Application-Id but get no role inference — §7). One number's Diameter joins into one flow through the MSISDN the wire states in international form (Sh `MSISDN`, Cx `Public-Identity`, Rf/Gy E.164 `Subscription-Id`); local forms are never completed with a country code. The charging ID (ICID) is shown but never joins flows — it belongs to caller and callee at once. Raw exports with no IP layer (link type USER 0) are detected and mapped, §8 |
| `dns` (ENUM only) | ✅ ENUM NAPTR lookups under `e164.arpa` (2026-09-13): the query name is the number written backwards, so the lookup joins the flow of that number's HSS and charging messages. NXDOMAIN is a routing answer, not a failure. Other DNS is not collected. Its own *ENUM (DNS)* tab |
